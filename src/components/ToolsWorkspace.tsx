...
import type { ReportSection } from '@/context/ReportContext';
import type { CalcProps } from '@/pages/tools/_shared';
import Affordability from '@/pages/tools/Affordability';
import RentVsBuy from '@/pages/tools/RentVsBuy';
import Amortization from '@/pages/tools/Amortization';
import DTI from '@/pages/tools/DTI';
import RefiBreakEven from '@/pages/tools/RefiBreakEven';
import ExtraPayment from '@/pages/tools/ExtraPayment';
import Buydown from '@/pages/tools/Buydown';
import VaEntitlement from '@/pages/tools/VaEntitlement';
import SellerNet from '@/pages/tools/SellerNet';
import CashOut from '@/pages/tools/CashOut';
import CountyIncome from '@/pages/tools/CountyIncome';
...
interface ToolsWorkspaceProps {
  /** Tool keys to hide (e.g. the agent portal hides the auth-gated census tool). */
  exclude?: string[];
  /** Build the branded report PDF for the collected sections. When omitted, the report
   *  builder bar is hidden entirely. */
  downloadReport?: (payload: { preparedFor: string; sections: ReportSection[] }) => Promise<Blob>;
}

/** The shared Tools experience: a grid of calculators (each opens in a modal) plus an
 *  optional report builder. Used by the loan-officer Tools page and the agent portal. */
export function ToolsWorkspace({ exclude, downloadReport }: ToolsWorkspaceProps) {
  const tools = exclude?.length ? TOOLS.filter((t) => !exclude.includes(t.key)) : TOOLS;
  ...
  const onDownload = async () => {
    if (!downloadReport || !sections.length) return;
    setBusy(true);
    setErr('');
    try {
      const blob = await downloadReport({ preparedFor: preparedFor.trim(), sections });
  ...
  return (
    <>
      {downloadReport && sections.length > 0 && (
        <div className="mb-6 rounded-[14px] border border-[rgba(45,212,191,0.3)] bg-[rgba(45,212,191,0.06)] p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {sections.map((s) => (
              <span key={s.key} className="inline-flex items-center gap-1.5 rounded-full border border-border-seg bg-card px-2.5 py-1 text-[12px] text-text-soft">
                {s.title}
          ...
          <div className="mt-3 max-w-[360px]">
            <input
              ...
              placeholder="Prepared for (client name — optional)"
