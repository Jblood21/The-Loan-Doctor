...
const YEAR = String(process.env.CENSUS_YEAR || '2023');
...
router.get('/counties', requireAuth, async (req, res) => {
  ...
  try {
    const rows = await censusFetch(`${BASE}?get=NAME&for=county:*&in=state:${state}${KEY}`);
    // rows[0] is the header; each data row is [NAME, state, county].
    const counties = (Array.isArray(rows) ? rows.slice(1) : [])
      .map((r) => ({ fips: fips3(r[2]), name: String(r[0] || '').replace(/,.*$/, '').trim() }))
      .filter((c) => c.fips && c.name)
      .sort((a, b) => a.name.localeCompare(b.name));
    res.json({ year: YEAR, counties });
  } catch {
...
router.get('/income', requireAuth, async (req, res) => {
  ...
  try {
    const rows = await censusFetch(`${BASE}?get=NAME,B19013_001E,B19013_001M&for=county:${county}&in=state:${state}${KEY}`);
